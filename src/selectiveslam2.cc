...
// Note: We no longer need cv::calcOpticalFlowPyrLK, so no include for cv::video.hpp
...
// -----------------------------------------------------------------------------
// Global variables for ROI selection and object tracking
...
// For object selection and tracking.
// When the user releases the mouse button, the drawn box is locked in as the object.
bool objectSelected = false;
cv::Rect selectedObjectBox;
cv::Mat selectedTemplate; // Template for tracking the object
std::mutex object_mutex;

// Mouse callback: update currentBox on drag; on LBUTTONUP, lock in selection.
void mouseCallback(int event, int x, int y, int flags, void* userdata) {
    ...
    if (event == cv::EVENT_LBUTTONDOWN) {
        ...
        if (currentBox.area() > 0) {
            std::lock_guard<std::mutex> lockObj(object_mutex);
            selectedObjectBox = currentBox;
            objectSelected = true;
            selectedTemplate.release(); // Clear any previous template.
            std::cout << "Object selected: " << selectedObjectBox << std::endl;
        }
    }
}

// -----------------------------------------------------------------------------
// Modified mono_tracking_zed: full-frame SLAM while object tracking uses template matching.
int mono_tracking_zed(const std::shared_ptr<stella_vslam::system>& slam,
    ...
    // Optionally load a mask image if provided (unused in this mode).
    ...
    // Create an OpenCV window for ROI and object visualization.
    ...
    // ROI visualization and object tracking thread using template matching.
    std::thread roi_visualization_thread([&]() {
        while (is_not_end) {
            ...
            if (!frame.empty()) {
                ...
                // Show the drawn box if object not yet locked in.
                {
                    std::lock_guard<std::mutex> lock(box_mutex);
                    if (!objectSelected && currentBox.area() > 0) {
                        cv::rectangle(displayFrame, currentBox, cv::Scalar(0, 255, 0), 2);
                    }
                }

                // If an object has been selected, use template matching to track it.
                {
                    std::lock_guard<std::mutex> lock(object_mutex);
                    if (objectSelected) {
                        cv::Mat gray;
                        cv::cvtColor(frame, gray, cv::COLOR_BGR2GRAY);
                        // If the template is not yet set, initialize it from the selected region.
                        if (selectedTemplate.empty()) {
                            cv::Rect validBox = selectedObjectBox & cv::Rect(0, 0, gray.cols, gray.rows);
                            if (validBox.area() > 0)
                                selectedTemplate = gray(validBox).clone();
                        } else {
                            // Run template matching on the current frame.
                            cv::Mat result;
                            int result_cols = gray.cols - selectedTemplate.cols + 1;
                            int result_rows = gray.rows - selectedTemplate.rows + 1;
                            result.create(result_rows, result_cols, CV_32FC1);
                            cv::matchTemplate(gray, selectedTemplate, result, cv::TM_CCOEFF_NORMED);
                            double minVal, maxVal;
                            cv::Point minLoc, maxLoc;
                            cv::minMaxLoc(result, &minVal, &maxVal, &minLoc, &maxLoc);
                            // Update the object's bounding box based on the best match.
                            selectedObjectBox = cv::Rect(maxLoc.x, maxLoc.y, selectedTemplate.cols, selectedTemplate.rows);
                            cv::rectangle(displayFrame, selectedObjectBox, cv::Scalar(255, 0, 0), 2);
                        }
                    }
                }
