...
// -----------------------------------------------------------------------------
// Modified mono_tracking_zed: full-frame SLAM while multi-object tracking uses template matching.
    ...
    std::thread slam_thread([&]() {
        while (is_not_end) {
            ...
            if (zed.grab() == sl::ERROR_CODE::SUCCESS) {
                ...
                cv::Mat rgb_image(zed_image.getHeight(), zed_image.getWidth(), CV_8UC4,
                                  zed_image.getPtr<sl::uchar1>(sl::MEM::CPU));
                cv::Mat color_frame;
                cv::cvtColor(rgb_image, color_frame, cv::COLOR_BGRA2BGR);

                cv::Mat depth_frame(zed_depth.getHeight(), zed_depth.getWidth(), CV_32FC1,
